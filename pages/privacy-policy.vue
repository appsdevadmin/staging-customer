  ...
  <div>
    ...
    <div class="legal-content intro">
      ...
      <div class="legal-content__body full">
        <div
          v-if="pageData.data.page_content[0].title"
          class="legal-content__sub-title"
        >
...
export default {
  asyncData({ store, error }) {
    const fetchPrivacyPolicyPage = store.dispatch('fetchWidgets', {
      page: 'privacy-policy-page'
    })

    return Promise.all([fetchPrivacyPolicyPage])
      .then((responses) => {
  ...
  computed: {
    meta() {
      return this.mxMetaUtils({
        title:
          this.pageData.data?.seo?.metaTitle ?? 'NNPC LTD | Privacy Policy',
        description:
        ...
        url: this.pageData.data?.seo?.canonicalURL
          ...
          : `${process.env.siteURL}/privacy-policy`,
  ...
  methods: {
    toggleAccordion(e) {
      ...
      e.currentTarget
        .querySelector('.accordion__control.y')
